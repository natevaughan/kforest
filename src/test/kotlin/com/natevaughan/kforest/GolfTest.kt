...
class GolfTest {
    ...
    companion object {
        val log = LoggerFactory.getLogger(GolfTest::class.java)
    }
    ...
    fun buildTreeTest() {
        val dataset = makeDataset()
        val root: CategoricalNode = buildTree(dataset, GOLF, emptyList(), 3, 0.05) as CategoricalNode
        assertNotNull(root)
    ...
    fun getCountsTest() {
        ...
        val entropies = getEntropyTuples(matrix, dataset.size)
        assertEquals(4, entropies.size)
    }
    
    @Test
    fun buildNodeTest(){
        val matrix: Map<String, Collection<Tuple2<String, Int>>> = mapOf(
            OVERCAST to listOf(
                    Tuple2(YES, 4)
            ),
            RAINY to listOf(
